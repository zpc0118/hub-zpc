...
import os
os.environ["FAISS_USE_CPU_ONLY"] = "1"  # Apple Silicon MPS 偶发卡死，强制 CPU
import json
import time
import logging
import numpy as np
...
BASE_DIR        = Path(__file__).parent.parent   # week10/
CHUNKS_DIR      = BASE_DIR / "data" / "chunks"
VECTORSTORE_DIR = Path(__file__).parent / "vectorstore"  # homework_rag/vectorstore/
...
# 断点续传：每 N 批存一次 checkpoint
CHECKPOINT_INTERVAL = 50              # 每 50 批保存一次
CHECKPOINT_FILE     = VECTORSTORE_DIR / ".embed_checkpoint.json"
CHECKPOINT_NPY      = VECTORSTORE_DIR / ".embed_checkpoint.npy"
...
def embed_texts(client: OpenAI, texts: list[str], show_progress: bool = True) -> np.ndarray:
    """
    批量计算 embedding，每批最多 10 条，支持断点续传。
    每 50 批自动保存 checkpoint，重跑时自动恢复。
    返回 shape=(N, EMBED_DIM) 的 float32 数组，已 L2 归一化。
    """
    total_batches = (len(texts) + BATCH_SIZE - 1) // BATCH_SIZE

    # ── 断点续传：检查是否有未完成的 checkpoint ──
    if CHECKPOINT_NPY.exists() and CHECKPOINT_FILE.exists():
        with open(CHECKPOINT_FILE, encoding="utf-8") as f:
            ckpt = json.load(f)
        completed = ckpt.get("completed_batches", 0)
        all_embeddings = list(np.load(CHECKPOINT_NPY))
        start_batch = completed
        logger.info(f"从 checkpoint 恢复: 已完成 {completed}/{total_batches} 批，从第 {completed+1} 批继续")
    else:
        all_embeddings = []
        start_batch = 0
        # 清理可能的残留文件
    ...
    for batch_idx in range(start_batch, total_batches):
        ...
        if show_progress and (batch_idx + 1) % 100 == 0:
            logger.info(f"  Embedding 进度: {batch_idx + 1}/{total_batches} 批")
        ...
        if (batch_idx + 1) % CHECKPOINT_INTERVAL == 0:
            ...
            np.save(CHECKPOINT_NPY, arr)
            with open(CHECKPOINT_FILE, "w", encoding="utf-8") as f:
                json.dump({"completed_batches": batch_idx + 1, "total_batches": total_batches}, f)
            logger.info(f"  Checkpoint: {batch_idx + 1}/{total_batches} 批已保存")
